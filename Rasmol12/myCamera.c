...
void
initializeCamera(PDB *pdb){
    if(pdb==NULL){
	fprintf(stderr,"Error: PDB data isn't exist\n");
	return;
    }
    
    //PDBファイルの情報からCenterを設定する
    center[0] = double(PDB->Center.x);
    center[1] = double(PDB->Center.y);
    center[2] = double(PDB->Center.z);

    //eyeの設定
    eye[0] = center[0];
    eye[1] = center[1];
    eye[2] = center[2] + pdb->maxRadius * 2.0;
    ...
    __camera.Fovy = 45.0;
    __camera.Aspect = 1.0;
    __camera.ClipNear = distance / 10.0;
    __camera.ClipFar = distance * 10.0;
